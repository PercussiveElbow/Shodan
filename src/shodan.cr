...
module Shodan
  class Client
    ...
    def scan(ips : Array(String))
      ...
      response = HTTP::Client.post("https://api.shodan.io/shodan/scan?key=#{@api_key}", form: "ips=#{ips_to_scan.to_s}") # todo
      Client.check_status_code(response.status_code)
      begin
        return StartScan.from_json(response.body)
      rescue ex : JSON::Error
        raise ShodanClientException.new(ex.to_s)
      end
    end
    ...
    def scan_status(scan_id : String)
      ...
      Client.check_status_code(response.status_code)
      begin
        return Scan.from_json(response.body)
      rescue ex : JSON::Error
        raise ShodanClientException.new(ex.to_s)
      end
    end
